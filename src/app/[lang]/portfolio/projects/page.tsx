...
import { ArrowLeft, Calendar, Building2, CheckCircle, ExternalLink, ChevronLeft, ChevronRight, Clock, Users } from 'lucide-react';
import { Locale } from "@/lib/i18n";
import { getDictionary } from "@/content/dictionary";
import { ProjectApiResponse, Project } from '@/types/project';

interface ProjectDetailQueryClientProps {
  params: Promise<{ lang: string }>;
}

function ProjectDetailContent({ params }: ProjectDetailQueryClientProps) {
  ...
  const fetchProject = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://merkwave.com/api/projects/get_detail.php?slug=${encodeURIComponent(slug)}`);
  ...
  const challenge = locale === 'ar' ? project.challenge_ar : project.challenge_en;
  const solution = locale === 'ar' ? project.solution_ar : project.solution_en;
  
  // Parse JSON arrays
  const technologies = project.technologiesEn && project.technologiesAr 
    ? (locale === 'ar' ? JSON.parse(project.technologiesAr || '[]') : JSON.parse(project.technologiesEn || '[]'))
    : [];
  
  const keyFeatures = project.key_features_en && project.key_features_ar
    ? (locale === 'ar' ? JSON.parse(project.key_features_ar || '[]') : JSON.parse(project.key_features_en || '[]'))
    : [];
  
  const resultsAchieved = project.results_achieved_en && project.results_achieved_ar
    ? (locale === 'ar' ? JSON.parse(project.results_achieved_ar || '[]') : JSON.parse(project.results_achieved_en || '[]'))
    : [];
            ...
            <div className={`flex flex-wrap justify-center gap-6 text-sm text-gray-400 ${isArabic ? 'flex-row-reverse' : ''}`}>
              ...
              {project.project_date && (
                <div className="flex items-center">
                  ...
                  {locale === 'ar' ? 'تاريخ الإكمال' : 'Completed'}: {new Date(project.project_date).toLocaleDateString(locale === 'ar' ? 'ar-EG' : 'en-US', { year: 'numeric', month: 'long' })}
                </div>
              )}
              {project.featured && (
                <div className="flex items-center">
                  <CheckCircle className="w-4 h-4 mr-2 text-yellow-400" />
                  <span className="text-yellow-400">{locale === 'ar' ? 'مشروع مميز' : 'Featured'}</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="bg-[#0a1628]/60 backdrop-blur-xl border border-gray-600/40 rounded-2xl p-8">
                ...
                <ul className="space-y-3">
                  {resultsAchieved.map((result: string, index: number) => (
                    <li key={index} className={`flex items-start ${isArabic ? 'flex-row-reverse text-right' : ''}`}>
                      <CheckCircle className={`w-5 h-5 text-green-400 ${isArabic ? 'ml-3' : 'mr-3'} mt-0.5 flex-shrink-0`} />
              ...
              <div className="bg-[#0a1628]/60 backdrop-blur-xl border border-gray-600/40 rounded-2xl p-8">
                ...
                <ul className="space-y-3">
                  {keyFeatures.map((feature: string, index: number) => (
                    <li key={index} className={`flex items-start ${isArabic ? 'flex-row-reverse text-right' : ''}`}>
                      <CheckCircle className={`w-5 h-5 text-yellow-400 ${isArabic ? 'ml-3' : 'mr-3'} mt-0.5 flex-shrink-0`} />
          ...
          {project.demoUrl && (
            <div className="text-center mt-12">
              ...
              >
                <span>{locale === 'ar' ? 'زيارة المشروع' : 'Visit Project'}</span>
          ...
          <div className="text-center bg-gradient-to-r from-cyan-500/10 via-blue-500/10 to-indigo-500/10 backdrop-blur-xl border border-cyan-400/20 rounded-3xl p-12 max-w-4xl mx-auto">
            <h2 className="text-4xl md:text-5xl font-bold mb-6">
              <span className="bg-gradient-to-r from-cyan-400 via-blue-400 to-indigo-400 bg-clip-text text-transparent">
                {locale === 'ar' ? 'لديك مشروع مماثل؟' : 'Have a Similar Project?'}
              </span>
            </h2>
...
export default function ProjectDetailPage({ params }: ProjectDetailQueryClientProps) {
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-gradient-to-br from-[#0a1628] via-[#0f2027] to-[#203a43]">
        <div className="container mx-auto px-4 py-20">
