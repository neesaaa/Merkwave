    ...
    return (
      <section className={`relative h-screen bg-[#0B192A] overflow-hidden flex items-center z-0`} dir={isArabic ? 'rtl' : 'ltr'}>
        ...
        <div className=" flex flex-col lg:flex-row px-2 sm:px-1 lg:px-0 relative  w-full h-full items-center justify-center">
          ...
          <div className={`flex flex-col justify-center w-full md:w-full lg:w-[70%] h-1/2 lg:h-full self-center  gap-6  items-center rounded-xl z-[9999] px-1  md:max-w-[80%]  ${isArabic ? "xl:pr-40 lg:pr-80 md:pr-3 sm:pr-1" : "xl:pl-40 lg:pl-80 md:pl-2"}`}>
            {/* Left content */}
            <motion.div 
            ...
            >
              ...
              {/* CTA Buttons */}
              <motion.div 
                className={`flex flex-col sm:flex-row  w-full  z-50 justify-start gap-2 lg:gap-4 items-start`}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                <Link href={`/${lang}/contact`}>
                  <motion.button
                      className="group relative inline-flex items-center justify-center  px-6 md:px-8 py-4  bg-gradient-to-r from-[#F9FF00] to-[#FFD000] text-black font-bold text-xs sm:text-sm xl:text-lg   rounded-full shadow-lg shadow-orange-400/30 overflow-hidden"
                ...
                <Link href={`/${lang}/services`}>
                  <motion.button
                    className="inline-flex items-center z-50 justify-center px-8 py-4  border-2 bg-gradient-to-r from-[#FF4D4D] to-[#C40000]  text-black font-semibold text-xs sm:text-sm xl:text-lg rounded-full  hover:text-black hover:shadow-[0_8px_20px_rgba(255,77,77,0.35)]
